This directory contains integration tests that test bitcoinbased and its
...
bitcoinbased and bitcoinbase-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (bitcoinbase-tx, ...).
...
Before tests can be run locally, Bitcoin Base Core must be built.  See the [building instructions](/doc#building) for help.
...
Many Bitcoin Base specific tests require bitcoinbase_hash. To install it:

- Clone the repo `git clone https://github.com/bitcoin-base/bitcoinbase_hash`
- Install bitcoinbase_hash `cd bitcoinbase_hash && pip3 install -r requirements.txt .`
...
The P2P and RPC ports used by the bitcoinbased nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another bitcoinbased
process running on the system (perhaps from a previous test which hasn't successfully
killed all its bitcoinbased nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other bitcoinbased processes are running.

On linux, the test framework will warn if there is another
bitcoinbased process running when the tests are started.

If there are zombie bitcoinbased processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
bitcoinbased processes running on the system, so should not be used if any non-test
bitcoinbased processes are being run.**

```bash
killall bitcoinbased
```

or

```bash
pkill -9 bitcoinbased
```
...
sure bitcoinbased processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and bitcoinbased `debug.log`s will all be dumped
...
`test_framework.log` and bitcoinbased `debug.log`s can be combined into a single
...
call methods that interact with the bitcoinbased nodes-under-test.

If further introspection of the bitcoinbased instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/bitcoinbased.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/bitcoinbased <pid>
```
