...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        with open(os.path.join(self.nodes[0].datadir, 'bitcoinbase.conf'), 'a', encoding='utf-8') as conf:
        ...
        self.nodes[0].assert_start_raises_init_error(
            expected_msg='Error: Error parsing command line arguments: Invalid parameter -bitcoinbase_cli=1',
            extra_args=['-bitcoinbase_cli=1'],
        )
        with open(inc_conf_file_path, 'w', encoding='utf-8') as conf:
            conf.write('bitcoinbase_conf=1\n')

        with self.nodes[0].assert_debug_log(expected_msgs=['Ignoring unknown configuration value bitcoinbase_conf']):
            self.start_node(0)
        self.stop_node(0)
        ...
        main_conf_file_path = os.path.join(self.options.tmpdir, 'node0', 'bitcoinbase_main.conf')
    ...
    def run_test(self):
        ...
        conf_file = os.path.join(default_data_dir, "bitcoinbase.conf")
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_bitcoinbase.conf")
