...
TMPDIR_PREFIX = "bitcoinbase_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bitcoinbaseds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bitcoinbaseds after the test execution")
        ...
        parser.add_argument("--bitcoinbased-arg", dest="bitcoinbased_extra_args", default=[], action="append",
                            help="Pass extra args to all bitcoinbased instances")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "bitcoinbased": ("bitcoind", "BCBD"),
            "dash-cli": ("bitcoincli", "BCBCLI"),
            "dash-util": ("bitcoinutil", "BCBUTIL"),
            "dash-wallet": ("bitcoinwallet", "BCBWALLET"),
        }
    ...
    def setup(self):
        ...
        self.extra_args_from_options = self.options.bitcoinbased_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bitcoinbaseds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bitcoinbased', self.options.bitcoind) for v in versions]
    ...
    def dynamically_initialize_datadir(self, mnidx):
        ...
        write_config(os.path.join(new_data_dir, "bitcoinbase.conf"),
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitcoinbased's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bitcoinbase.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if bitcoinbased has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitcoinbased has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if bitcoinbased has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitcoinbased has not been built with zmq enabled.")
...
class DashTestFramework(BitcoinTestFramework):
    ...
    def set_bitcoinbase_test_params(self, num_nodes, masterodes_count, extra_args=None, evo_count=0):
    ...
    def set_bitcoinbase_llmq_test_params(self, llmq_size, llmq_threshold):
