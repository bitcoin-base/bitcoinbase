...
Deterministic (HD) wallets by default. This change aligns the behavior of `bitcoinbased` and `bitcoinbase-qt` with the previously
...
Modified the Dockerfile to exclude the `bitcoinbase-qt` graphical wallet interface from the set of binaries copied to
`/usr/local/bin` during the Docker image build process. This change streamlines the Docker image, making it more
suitable for server environments and headless applications, where the graphical user interface of `bitcoinbase-qt` is not
...
- New Discreet mode for hiding balances in bitcoinbase-qt
- Added a "Close all wallet" menu item for bitcoinbase-qt
