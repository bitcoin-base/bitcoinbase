When Bitcoin Base Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Bitcoin Base node is listening,
or one where a service is listening that is not related to the Bitcoin Base network.
As a result, this service may occasionally get connection attempts from Bitcoin Base
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Bitcoin Base Core,
trying to connect because it thinks there is a Bitcoin Base node on that
...
They are also considered "bad" ports as they require clients to either run Bitcoin Base
...
Below is a list of "bad" ports which Bitcoin Base Core avoids when choosing a peer to
