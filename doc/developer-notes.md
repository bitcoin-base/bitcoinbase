...
- [Development guidelines](#development-guidelines)
    - [General Bitcoin Base Core](#general-bitcoinbase-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bitcoin Base Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while bitcoinbased is running with the `logging`
RPC.  For instance, launching bitcoinbased with `-debug` or `-debug=1` will turn on
...
Bitcoin Base Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run bitcoinbased.

You can then use the `-debug=lock` configuration option at bitcoinbased startup or
...
([`valgrind.supp`](https://github.com/bitcoin-base/bitcoinbase/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bitcoinbase
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bitcoinbase --log_level=test_suite
$ valgrind -v --leak-check=full src/bitcoinbased -printtoconsole
...
# A coverage report will now be accessible at `./test_bitcoinbase.coverage/index.html`.
```
...
To profile a running bitcoinbased process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep bitcoinbased` -- sleep 60
```
...
Bitcoin Base Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`bitcoinbased`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as Bitcoin Base Core, where everyone uses
...
pay attention to for reviewers of Bitcoin Base Core code.

General Bitcoin Base Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Bitcoin Base Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Bitcoin Base Core, in which case
...
a Bitcoin Base Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Bitcoin Base Core, the wallet was always
...
- Use *invalid* Bitcoin Base addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
