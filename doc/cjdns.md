# CJDNS support in Bitcoin Base Core

It is possible to run Bitcoin Base Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bitcoin Base Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bitcoin Base network and individual nodes.
...
before it will work with your Bitcoin Base Core node. This is described in steps
...
## Run Bitcoin Base Core with CJDNS

Once you are connected to the CJDNS network, the following Bitcoin Base Core
...
When enabled, this option tells Bitcoin Base Core that it is running in an
...
IPv6 local network. This helps Bitcoin Base Core perform better address management:
...
CJDNS support was added to Bitcoin Base Core in version 21.0 and there may be fewer
...
[doc/tor.md](https://github.com/bitcoin-base/bitcoinbase/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Bitcoin Base Core

There are several ways to see your CJDNS address in Bitcoin Base Core:
