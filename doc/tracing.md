# User-space, Statically Defined Tracing (USDT) for Bitcoin Base Core

Bitcoin Base Core includes statically defined tracepoints to allow for more
...
**Note**: Bitcoin Base Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bitcoin Base Core
...
Multiple tools can list the available tracepoints in a `bitcoinbased` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Bitcoin Base Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bitcoinbased
stap net        outbound_message 0x0000000000107c05 /src/bitcoinbased
stap validation block_connected  0x00000000002fb10c /src/bitcoinbased
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Bitcoin Base Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitcoinbased | grep NT_STAPSDT -A 4 -B 2
