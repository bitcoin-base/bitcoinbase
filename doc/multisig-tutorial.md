...
Currently, it is possible to create a multisig wallet using Bitcoin Base Core only.
...
```bash
./src/bitcoinbased -testnet -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in Bitcoin Base Core. For this, an external signer/xpub can be used.
...
Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Bitcoin Base Core single-sig wallets are created using path `m/44'/1'/0'/` for PKH based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Bitcoin Base Core will include the checksum in their output.
...
PSBT is a data format that allows wallets and other tools to exchange information about a Bitcoin Base transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]
