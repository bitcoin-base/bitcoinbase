...
The data directory is the default location where the Bitcoin Base Core files are stored.
...
3. All content of the data directory, except for `bitcoinbase.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Bitcoin Base blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `bitcoinbase.conf`        | User-defined [configuration settings](dash-conf.md) for `bitcoinbased` or `bitcoinbase-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `bitcoinbased.pid`        | Stores the process ID (PID) of `bitcoinbased` or `bitcoinbase-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `bitcoinbased` or `bitcoinbase-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [bitcoinbase.conf](dash-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`bitcoinbase-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Bitcoin Base Core:

Path           | Description | Repository notes
---------------|-------------|-----------------
`banlist.dat`  | Stores the addresses/subnets of banned nodes; completely ignored and superseded by `banlist.json` in 20.0 | [PR #5574](https://github.com/bitcoin-base/bitcoinbase/pull/5574)
