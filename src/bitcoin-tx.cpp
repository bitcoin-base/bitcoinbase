...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinbase-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  bitcoinbase-tx [options] <hex-tx> [commands]  Update hex-encoded dash transaction\n"
                "or:     bitcoinbase-tx [options] -create [commands]   Create hex-encoded dash transaction\n"
                "\n";
