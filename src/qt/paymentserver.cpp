...
const QString BITCOIN_IPC_PREFIX("bcb:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bcb: URI
        {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click bcb: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start bcb: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling bcb: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("bcb://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'bcb://' is not a valid URI. Use 'bcb:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bcb: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Bitcoin Base address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
