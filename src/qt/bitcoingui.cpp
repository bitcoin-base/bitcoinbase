...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Bitcoin Base address"));
    ...
    coinJoinCoinsAction->setStatusTip(tr("Send %1 funds to a Bitcoin Base address").arg(strCoinJoinName));
    coinJoinCoinsAction->setToolTip(coinJoinCoinsAction->statusTip());

    receiveCoinsAction = new QAction(tr("&Receive"), this);
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bcb: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Bitcoin Base addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Bitcoin Base addresses"));
    ...
    openAction->setStatusTip(tr("Open a bcb: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bitcoin Base command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        ...
        tooltip = tr("%n active connection(s) to Bitcoin Base network", "", count);
    } else {
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the bitcoinbase logo
