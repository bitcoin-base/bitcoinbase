This directory contains the source code for the Bitcoin Base Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Bitcoin Base UI.
...
- ClientModel is used by the main application `bitcoinbasegui` and several models like `peertablemodel`.
...
#### paymentserver.(h/cpp)
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bcb:... link from a browser).

Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a bcb:... link from a browser).
...
**Note:** Do not change `local/bitcoinbase_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bitcoinbase-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bitcoinbase-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bitcoinbase-qt`
9. You're all set! Start developing, building, and debugging the Bitcoin Base Core GUI
