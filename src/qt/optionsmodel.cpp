...
// Copyright (c) 2014-2024 The BitcoinBase Core developers
...
void OptionsModel::Init(bool resetSettings)
{
    ...
    // Display
    if (!settings.contains("DisplayBitcoinBaseUnit")) {
        settings.setValue("DisplayBitcoinBaseUnit", QVariant::fromValue(BitcoinUnit::BCB));
    }
    QVariant unit = settings.value("DisplayBitcoinBaseUnit");
    if (unit.canConvert<BitcoinUnit>()) {
        m_display_bitcoin_unit = unit.value<BitcoinUnit>();
    } else {
        m_display_bitcoin_unit = BitcoinUnit::BCB;
        settings.setValue("DisplayBitcoinBaseUnit", QVariant::fromValue(m_display_bitcoin_unit));
    }
