 ...
 *  We have 4 times smaller block times in Bitcoin Base, so we need to push 4 times more invs per 1MB. */
...
struct Peer {
     ...
     * (Bitcoin Base)    Enables the flag that allows GetTxRelay() to return m_tx_relay */
...
private:
     ...
     * (Bitcoin Base)    Always initialized but selectively available through GetTxRelay()
...
bool PeerManagerImpl::AlreadyHave(const CInv& inv)
{
    ...
    /*
        Bitcoin Base Related Inventory Messages
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        // BCB this code also handles non-TXs (Bitcoin Base specific messages)
