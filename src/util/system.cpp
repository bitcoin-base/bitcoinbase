...
const char * const BITCOIN_CONF_FILENAME = "bitcoinbase.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinbase-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bitcoinbase
    // macOS: ~/Library/Application Support/Bitcoinbase
    // Unix-like: ~/.bitcoinbase
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoinbase";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bitcoinbase";
#else
    // Unix-like
    return pathRet / ".bitcoinbase";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty bitcoinbase.conf if it does not exist
