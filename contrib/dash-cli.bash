...
_bitcoinbase_rpc() {
    ...
    $bitcoinbase_cli "${rpcargs[@]}" "$@"
}

_bitcoinbase_cli() {
    local cur prev words=() cword
    local bitcoinbase_cli
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($bitcoinbase_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_bitcoinbase_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _bitcoinbase_cli dash-cli
