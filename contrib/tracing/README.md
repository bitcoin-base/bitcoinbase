...
Tracing (USDT) support for Bitcoin Base Core on Linux using. For more information on
USDT support in Bitcoin Base Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `bitcoinbased` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `bitcoinbased` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Bitcoin Base Core USDT documentation] on how to list available tracepoints in your
`bitcoinbased` binary.

[Bitcoin Base Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/bitcoinbased
```
...
Bitcoin Base P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/bitcoinbased
```
...
In a different terminal, starting Bitcoin Base Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/bitcoinbased -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/bitcoinbased
```
