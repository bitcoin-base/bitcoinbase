...
ARG BCB_HASH_VERSION=1.4.0
RUN set -ex; \
    cd /tmp; \
    git clone --depth 1 --no-tags --branch=${BCB_HASH_VERSION} https://github.com/bitcoin-base/bitcoinbase_hash; \
    cd bitcoinbase_hash && pip3 install -r requirements.txt .; \
    cd .. && rm -rf bitcoinbase_hash
