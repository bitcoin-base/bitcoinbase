# Bitcoin Base Core Development Guide

## Overview

Bitcoin Base Core is the reference implementation for Bitcoin Base, a cryptocurrency. It builds on top of Bitcoin Core, a codebase that
is primarily written in C++20 (requiring at least Clang 16 or GCC 11.1). Bitcoin Base Core uses the GNU Autotools build system.
...
- **Implementation** `src/` - C++20 codebase
  ...
  - `src/qt/` - Implementation of Bitcoin Base Qt, the GUI (uses Qt 5)
...
- **Functional Tests**: `test/functional/` - Python tests (minimum version in `.python-version`) dependent on `bitcoinbased` and `dash-node`
...
./src/test/test_bitcoinbase --run_test=getarg_tests

# Debug unit tests
gdb ./src/test/test_bitcoinbase
```
...
Bitcoin Base Core extends Bitcoin Core through composition, using a layered architecture:

```
Bitcoin Base Core Components
...
#### Bitcoin Base-Specific Databases

- **CFlatDB**: A Bitcoin Base-specific flat file database format used for persistent storage
...
- **CDBWrapper**: Bitcoin Core database wrapper extended for Bitcoin Base-specific data
...
2. **Parameter Interaction**: Bitcoin Base-specific configuration validation
3. **Interface Setup**: Bitcoin Base manager instantiation in NodeContext
...
- **ChainstateManager**: Enhanced with Bitcoin Base-specific validation
...
# Run bitcoinbased with debug logging
./src/bitcoinbased -debug=all -printtoconsole

# Run functional test with custom bitcoinbased
test/functional/test_runner.py --bitcoinbased=/path/to/bitcoinbased
...
# Memory debugging
valgrind --leak-check=full ./src/bitcoinbased
```
...
- Bitcoin Base uses `unordered_lru_cache` for efficient caching with LRU eviction
- The codebase extensively uses Bitcoin Base-specific data structures for performance
